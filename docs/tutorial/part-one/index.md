...
> 💡 基本的に、Gatsby 開発サーバーを実行している場合、Gatsby サイトファイルをバックグラウンドで「監視」しています。ファイルを保存すると、変更をすぐにブラウザに反映します。ページをハードリフレッシュしたり、開発サーバーを再起動したりする必要はありません。変更はすぐに表示されます。
...
![ヘッダーにデータを渡す](07-pass-data-header.png)
